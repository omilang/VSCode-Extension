...
const MODULE_MEMBERS = {
  ...
  files: [
    ...
    { label: 'open', insertText: 'open(${1:path}, ${2:"r"})', signature: 'open(path<string>, [mode<string>])', documentation: 'Opens a file and returns `file_handle`.' },
    { label: 'close', insertText: 'close(${1:handle})', signature: 'close(handle<file_handle>)', documentation: 'Closes an opened file handle. Safe to call multiple times.' },
    { label: 'read', insertText: 'read(${1:handle})', signature: 'read(handle<file_handle>, [count<number>])', documentation: 'Reads text from an opened handle. `count` is optional (default `-1`).' },
    { label: 'write', insertText: 'write(${1:handle}, ${2:data})', signature: 'write(handle<file_handle>, data<string>)', documentation: 'Writes text to an opened handle and returns written length.' },
  ],
  paths: [
